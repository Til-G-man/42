...
/*   Updated: 2024/08/18 14:52:51 by tilman           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_input(va_list args, const char *format)
{
	...
	if (*format == 'd' || *format == 'i')
		counter = ft_putnbr_count(va_arg(args, int), 1, 0);
	else if (*format == 'u')
		counter = ft_putunbr_count(va_arg(args, unsigned int), 1, 0);
	else if (*format == 's')
		counter = ft_putstr_count(va_arg(args, char *), 1);
	else if (*format == 'p')
		counter = ft_putptr_count((unsigned long) va_arg(args, void *));
	else if (*format == 'x')
		counter = ft_intohex(va_arg(args, unsigned int), 0, 0);
	else if (*format == 'X')
		counter = ft_intohex(va_arg(args, unsigned int), 1, 0);
	else if (*format == 'c')
		counter = ft_putchar_count(va_arg(args, int), 1);
	else
	{
